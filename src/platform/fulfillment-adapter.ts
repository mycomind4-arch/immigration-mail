/**
 * Immigration Mail adapter for @mailmypdf/payment-fulfillment
 *
 * Bridges the shared fulfillment contract to Immigration Mail's
 * Supabase-backed MailingIntentStore and MailMyPDF client.
 *
 * This replaces the duplicated fulfillment logic in
 * server/api/mail/response.ts and server/api/webhooks/stripe.ts
 * with a call to the shared fulfillment engine.
 */
...
import type {
  ...
  MailType,
  LegalReference,
} from "../../../packages/payment-fulfillment/src/index";

// ── Supabase Intent Store ────────────────────────────────────────────────────
...
function rowToIntent(row: Record<string, unknown>): MailingIntent {
  return {
    ...
    approved_draft_hash: (row.approved_draft_hash as string) ?? null,
    approved_recipient_hash: (row.approved_recipient_hash as string) ?? null,
