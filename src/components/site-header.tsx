...
export function SiteHeader() {
  ...
  return (
    <header className="sticky top-0 z-40 border-b border-rule/60 bg-paper/90 backdrop-blur-md">
      <div className="mx-auto flex min-h-14 max-w-6xl items-center justify-between px-4 py-2 sm:px-6">
        <Link to="/" className="flex items-center gap-2.5 group" aria-label="Immigration Mail home">
          ...
          <span className="flex flex-col leading-none">
            <span className="font-serif text-lg transition-colors group-hover:text-stamp">Immigration Mail</span>
        ...
        <div className="hidden items-center gap-1 md:flex">
          ...
          <Link to="/analyze" className="px-3 py-2 text-sm text-ink-soft hover:text-foreground">Analyze</Link>
          {user ? <>
          ...
          <Link to="/workflows/respond-to-notice" className="ml-2 inline-flex items-center gap-2 rounded-full bg-primary px-5 py-2 text-sm font-medium text-primary-foreground shadow-stamp transition-transform hover:-translate-y-0.5">Start a letter</Link>
        </div>
      ...
      {mobileOpen && <div className="border-t border-rule bg-paper md:hidden"><div className="flex flex-col gap-1 px-4 py-3">{navItems.map((item) => <Link key={item.label} to={item.href} className="rounded-md px-3 py-2.5 text-sm font-medium text-ink-soft hover:bg-muted/50" onClick={() => setMobileOpen(false)}>{item.label}</Link>)}<Link to="/analyze" className="rounded-md px-3 py-2.5 text-sm font-medium text-ink-soft hover:bg-muted/50" onClick={() => setMobileOpen(false)}>Analyze a document</Link>{user ? <><Link to="/cases" className="rounded-md px-3 py-2.5 text-sm font-medium text-ink-soft hover:bg-muted/50" onClick={() => setMobileOpen(false)}>My Cases</Link><Link to="/dashboard" className="rounded-md px-3 py-2.5 text-sm font-medium text-ink-soft hover:bg-muted/50" onClick={() => setMobileOpen(false)}>My Mailings</Link></> : <Link to="/auth" className="rounded-md px-3 py-2.5 text-sm font-medium text-ink-soft hover:bg-muted/50" onClick={() => setMobileOpen(false)}>Sign in</Link>}<div className="mt-2 border-t border-rule/60 pt-3"><p className="px-3 text-[10px] uppercase tracking-[0.2em] text-muted-foreground">MailMyPDF fulfillment</p><p className="px-3 pt-1 text-xs text-muted-foreground">Print, mail, track, and preserve proof.</p></div><Link to="/workflows/respond-to-notice" className="mt-2 inline-flex items-center justify-center gap-2 rounded-full bg-primary px-5 py-2.5 text-sm font-medium text-primary-foreground" onClick={() => setMobileOpen(false)}>Start a letter</Link></div></div>}
    </header>
  );
}
