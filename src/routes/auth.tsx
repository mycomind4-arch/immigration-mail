    ...
    <div className="min-h-screen">
      ...
      <main className="mx-auto max-w-4xl px-6 py-16">
        <div className="grid overflow-hidden rounded-2xl border border-rule md:grid-cols-2">
          <div className="p-8 md:p-10" style={{ background: "linear-gradient(135deg, oklch(0.25 0.05 260) 0%, oklch(0.2 0.04 258) 100%)" }}>
            <div className="postmark w-fit" style={{ borderColor: "rgba(255,255,255,.2)", color: "oklch(0.75 0.08 72)", background: "rgba(255,255,255,.05)" }}>Immigration Mail</div>
            <h1 className="mt-8 font-serif text-3xl text-white">Your correspondence, organized and sent.</h1>
            <p className="mt-4 text-sm leading-7 text-white/60">Create an account to save drafts, track mailings, and keep a permanent record of your correspondence.</p>
            <ul className="mt-8 space-y-3">
              {["Save and resume workflows", "Track all mailings in one place", "Keep proof of timely submission", "Re-use recipient addresses"].map((item) => (
                <li key={item} className="flex items-center gap-2 text-sm text-white/70">
                  <svg className="h-4 w-4 text-stamp" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" /></svg>
                  {item}
                </li>
              ))}
            </ul>
          ...
          <div className="flex flex-col justify-center bg-card p-8 md:p-10">
            {submitted ? (
              <div className="text-center">
                ...
                <h2 className="mt-5 font-serif text-2xl">You're on the list!</h2>
                <p className="mt-3 text-sm text-muted-foreground">We'll notify you at <span className="font-medium text-foreground">{email}</span> when accounts launch.</p>
